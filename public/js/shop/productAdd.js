...
$('#product_id').on('change', function() {
    var  productmrp =$(this).find(':selected').data("productmrp");
    $('#unit_price').val(productmrp);
});
...
function variationItemDesign() {
    ...
    markup += '<tr>';
        ...
        markup +='</td>';
        markup +='<td>';
             markup +='<input type="number" name="variation['+product_variation_count+'][discount_price]" placeholder="Discount Price" class="form-control form-control-sm change-productdiscountprice">';
        markup +='</td>';
        markup +='<td>';
...
$(document).on('keyup', '.change-productdiscountprice', function() {
    ...
    if(dotAndNumber(productDiscountPrice)) {
        if(productDiscountPrice.length > 15) {
            productDiscountPrice = lenChecker(productDiscountPrice, 15);
            $(this).val(productDiscountPrice);
        }
