                                    ...
                                    @if(!blank($banners))
                                        @foreach($banners as $banner)
                                            ...
                                            <tr data-id="{{ $banner->id}}">
                                                ...
                                                <td> <img alt="image" src="{{ $banner->images }}" class="img-thumbnail image-width mt-4 mb-3"> </td>
                                                <td>{{ Str::limit($banner->title, 60, '...') }} </td>
                                                <td> @if ($banner->status==5)
                                                    {{ ('Active') }}
                                                    @else
                                                    {{ ('Inactive') }}
                                                    @endif
                                                ...
                                                @if (auth()->user()->can('banner_edit') || auth()->user()->can('banner_delete'))
                                                    <td>
                                                        ...
                                                        <form class="float-left pl-2"
                                                            action="{{ route('admin.banner.destroy', $banner) }}" method="POST">
                                                            @csrf
                                                            @method('DELETE')
