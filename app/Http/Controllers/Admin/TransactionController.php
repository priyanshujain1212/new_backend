        ...
        if ( request()->ajax() ) {
            ...
            if ( !blank($transactions) ) {
                foreach ( $transactions as $transaction ) {
                    ...
                    $transaction->date      = food_date_format_with_day($transaction->created_at);
                    $transaction->from_user = $this->generateName($transaction, 'source');
                    $transaction->to_user   = $this->generateName($transaction, 'destination');
                    $transaction->amount    = $this->generateAmount($transaction);
                    $i++;
                }
            }

            return Datatables::of($transactions)->editColumn('id', function( $transaction ) {
