    ...
    public function index( Request $request )
    {
        ...
        if ( $_POST ) {
            ...
            if ( $role_id && $user_id ) {
                ...
                $this->data['creditBalances'] = User::role($role->name)->where('id', $user_id)->latest()->get();
            } elseif ( $user_id ) {
                $this->data['creditBalances'] = User::where('id', $user_id)->latest()->get();
            } elseif ( $role_id ) {
                $role                         = Role::find($role_id);
                $this->data['creditBalances'] = User::role($role->name)->latest()->get();
                $this->data['users']          = User::role($role->name)->latest()->get();
            } else {
                $this->data['creditBalances'] = User::all();
            }
        }
